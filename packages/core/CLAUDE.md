...
- **Zero runtime dependencies.** No I/O, no adapters, no frameworks. Pure types + a tiny capability helper.
...
- `src/domain/` — the canonical shapes (`sessions`, `agents`, `events`, `tools`, `costs`, `skills`, `channels`, `mcps`, `tickets`, `replay`, `common`).
- `src/contracts/` — adapter interfaces grouped by capability (`agent-adapter`, `session-ingest-adapter`, `replay-adapter`, `runtime-control-adapter`, `mcp-adapter`, `channel-adapter`, `pricing-adapter`, `common`).
...
- `capabilities.ts` — list + predicate for capability detection; keep in sync with `contracts/`.
...
- Tests live next to the code (`*.test.ts`). Currently only `capabilities.test.ts` exists.
