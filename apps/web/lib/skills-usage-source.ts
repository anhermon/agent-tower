...
/**
 * Server-only data layer for the Skills module. Extracts Skill-tool invocation
 * telemetry from locally stored Claude Code JSONL sessions and joins the
 * results against the discovered {@link SkillManifest} catalogue.
 *
...
export async function computeSkillsUsage(options?: {
  ...
  try {
    ...
    const report = buildReport(scan, skills);
...
async function scanWithCache(files: readonly ClaudeSessionFile[]): Promise<ScanResult> {
  const sortedKeyParts = files
    .map((file) => `${file.filePath}:${file.modifiedAt}`)
    .sort();
...
function buildReport(scan: ScanResult, skills: readonly SkillManifest[]): SkillsUsageReport {
  ...
  for (const s of skills) {
    ...
    if (typeof s.name === "string" && s.name.length > 0) {
      // First writer wins — if two skills share a frontmatter name, we keep
      // the earlier match so behaviour is deterministic per list ordering.
