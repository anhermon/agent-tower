...
import { type ListSessionsResult, listSessionsOrEmpty } from "@/lib/sessions-source";
import { type ListSkillsResult, listSkillsOrEmpty } from "@/lib/skills-source";
...
const _getOverviewState = unstable_cache(
  async (): Promise<OverviewState> => {
    const now = new Date();
    const [sessions, agents, skills]: [ListSessionsResult, ListAgentsResult, ListSkillsResult] =
      await Promise.all([listSessionsOrEmpty(), listAgentsOrEmpty(now), listSkillsOrEmpty()]);
...
export async function getOverviewState(): Promise<OverviewState> {
  return await _getOverviewState();
}
