...
/**
 * Read-only local-filesystem adapter for the Skills module. Mirrors the
 * conventions used by `sessions-source.ts`:
 ...
 *   - Each `SKILL.md` under a root becomes one {@link SkillManifest}.
 *   - No writes, no side effects. Results are cached in-process keyed by
 *     file path + mtime.
 */
...
export type ListSkillsResult =
  | { readonly ok: true; readonly skills: readonly SkillManifest[]; readonly roots: readonly ResolvedSkillsRoot[] }
  | {
      readonly ok: false;
...
function splitFrontmatter(raw: string): Frontmatter {
  // Only honour a frontmatter block when it is at the very top of the file.
  // Accept `\r\n` as well as `\n` separators to be tolerant of Windows files.
  const match = /^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/.exec(raw);
  if (!match) {
  ...
  try {
    ...
    // fall through: invalid YAML → treat the whole file as body so the page
...
function firstHeadingAbstract(body: string): string | null {
  // If there is no frontmatter description, fall back to the first paragraph
  // after the first markdown heading so the card still has something to say.
...
function extractTriggers(description: string): readonly string[] {
  ...
  // Anchor on a sentence that starts with "Trigger on" / "Trigger when" /
  // "Trigger proactively …" and harvest every quoted phrase inside it. This
  // avoids greedy matches that would reach into adjacent paragraphs.
