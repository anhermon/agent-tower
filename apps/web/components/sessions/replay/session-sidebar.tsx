...
function computeTokenStats(replay: ReplayData): TokenStats {
  let inputTokens = 0;
  let outputTokens = 0;
  let cacheRead = 0;
  let cacheWrite = 0;
...
function computeTopTools(replay: ReplayData): [string, number][] {
  ...
  return [...toolCounts.entries()].sort((a, b) => b[1] - a[1]).slice(0, 8);
}
          ...
          return (
            <div key={name} className="flex items-center gap-2">
              ...
              <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-white/[0.06]">
                <div className="h-full rounded-full bg-warn/60" style={{ width: `${width}%` }} />
              </div>
...
export function SessionSidebar({ replay }: Props) {
  const { inputTokens, outputTokens, cacheRead, cacheWrite } = computeTokenStats(replay);
  ...
  const topTools = computeTopTools(replay);
  ...
  return (
    <div className="space-y-6 text-sm">
      ...
      <TopToolsSection topTools={topTools} maxToolCount={maxToolCount} />
